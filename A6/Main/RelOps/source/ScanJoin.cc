...
ScanJoin :: ScanJoin (MyDB_TableReaderWriterPtr leftInputIn, MyDB_TableReaderWriterPtr rightInputIn,
	...
	if (leftInputIn->getNumPages () < rightInputIn->getNumPages ()) {
		...
		// This for loop takes every element in equalityChecksIn
		// And create a reverse pair then store into equalityChecks
		...
		// We need to make sure that left table is smaller
		hadToSwapThem = true;
	}
}
...
void ScanJoin :: run () {
	...
	// Create a iterator for all records
	// Then add to the hash table and check if it is true for predicate
